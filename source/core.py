...
class BillManager:
    ...
    def __init__(
        ...
        self.year = year
        self.month = month
        self.dateOfBill = self.getBillDateGerman(
            autoDate=autoDate, dateOfBill=dateOfBill
        )
        ...
        self.pathDictionary = os.path.join(
            self.pathMain, "templates", "dictionary.xlsx"
        )
    ...
    def createBills(self):
        ...
        self.tableOfDrivers = pd.read_excel(io=self.pathTableOfDrivers).set_index(
            "driver"
        )

        # Read dictionary and convert into python-dict
        self.dictionary = pd.read_excel(io=self.pathDictionary, header=None, skiprows=1)
        ...
        self.logbook["start"] = self.logbook.apply(
            func=lambda row: datetime.datetime.combine(
                row["dateStart"], row["timeStart"]
            ),
            axis=1,
        )

        self.logbook["end"] = self.logbook.apply(
            func=lambda row: datetime.datetime.combine(row["dateEnd"], row["timeEnd"]),
            axis=1,
        )
        ...
        self.logbookF["duration"] = self.logbookF.apply(
            func=lambda row: ((row["end"] - row["start"]).total_seconds()) / (60 * 60),
            # unit = hours
            axis=1,
        )
        ...
        self.logbookF["price"] = self.logbookF.apply(
            func=lambda row: self.priceFunction(
        ...
        self.summation = pd.DataFrame(
            [self.totalDistance, self.totalDuration, self.totalPrice]
        ).T
        ...
        for label in ["start", "end"]:
            self.overview[label] = self.overview[label].dt.strftime(self.format)

        ##################
        # Rename overview latex tables

        self.overviewR = self.overview.rename(columns=self.dictionary)
        self.overviewR = self.renameDataFrameIndexNames(self.overviewR, self.dictionary)

        self.dictionarySummation = {
            key: self.dictionary["total"] + " " + value
            for key, value in self.dictionary.items()
        }
        self.summationR = self.summation.rename(columns=self.dictionarySummation)
        ...
        self.overviewDict = {}
        self.overviewDict["overviewTable"] = self.overviewR.to_latex()
        self.overviewDict["summationTable"] = self.summationR.to_latex()
        ...
        self.nameLatexFile = "{y}_{m}_{name}.tex".format(
            y=str(self.year), m=str(self.month).zfill(2), name="overview"
        )

        # Excute
        self.createPdf(
            dirOutput=self.dirOutput,
            nameLatexFile=self.nameLatexFile,
            template=self.getTemplate(
                pathTemplate=os.path.join(self.pathMain, "templates", "overview.tex")
            ),
            latexDict=self.overviewDict,
            unnecessaryFileEndings=self.unnecessaryFileEndings,
        )

        ##################
        # Process drivers

        keys = ["firstName", "lastName", "street", "streetNumber", "postCode", "city"]
        ...
        for driver in self.activeDrivers:
            ...
            driverDict["pathSignature"] = os.path.relpath(
                ...
                "\\", "/"
            )  # Tex on Windows requires forward slash /
            for key in keys:
        ...
        for driver in self.activeDrivers:
            self.createPdf(
                ...
                nameLatexFile="{y}_{m}_{driver}.tex".format(
                    y=str(self.year), m=str(self.month).zfill(2), driver=driver
                ),
                template=self.getTemplate(
                    pathTemplate=os.path.join(
                        self.pathMain, "templates", "singleUser.tex"
                    )
                ),
    ...
    def plotPriceFunction(
        self, rangeDistance=[0, 120], durations=[23.99, 24, 24.01], ylim=[0, 45]
    ):
        import numpy as np
        ...
        for plotNumber, duration in enumerate(durations):
            ...
            y = list(
                map(lambda x: self.priceFunction(distance=x, duration=duration), x)
            )
            ax.plot(
                x,
                y,
    ...
    def createPdf(
        self, dirOutput, nameLatexFile, template, latexDict, unnecessaryFileEndings
    ):
        ...
        with cd(dirOutput):
            cmd = ["pdflatex", "-interaction", "nonstopmode", pathLatexFile]
            ...
            for ending in unnecessaryFileEndings:
                try:
                    os.unlink(os.path.splitext(pathLatexFile)[0] + ending)
    ...
    def renameDataFrameIndexNames(self, dataFrame, dictionary):
        ...
        if length == 1:
            dataFrame.index.rename(dictionary[dataFrame.index.name], inplace=True)
        else:
            dataFrame.index.rename(
                [dictionary[name] for name in dataFrame.index.names], inplace=True
            )
        return dataFrame
